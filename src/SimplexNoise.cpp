...
{ //adapted from inigo quilez's formula at https://iquilezles.org/articles/normalsSDF/

    //returns gradient as first 3 components, and noise value as 4th
    const float eps = 0.0001; // or some other value
    const glm::vec2 h = glm::vec2(eps, 0);

    const float current_noise = this->fractal(octaves, x, z);
    const float behind_noise = this->fractal(octaves, x, z - eps);
    const float front_noise = this->fractal(octaves, x, z + eps);
    const float left_noise = this->fractal(octaves, x - eps, z);
    const float right_noise = this->fractal(octaves, x + eps, z);


    return glm::vec4(glm::normalize(glm::vec3(left_noise - right_noise, eps * 2, behind_noise - front_noise)), current_noise);
}
